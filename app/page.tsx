...
export default async function Page() {
  const [settingsRes, nichesRes, leadsRes, statsRes] = await Promise.all([
    ...
    pool.query(`
      select
        count(*) as total,
        sum(case when status = 'sent' or status = 'followed_up' then 1 else 0 end) as sent_total,
        sum(case when initial_opened_at is not null or followup_opened_at is not null then 1 else 0 end) as opened_total,
        sum(case when initial_sent_at >= current_date then 1 else 0 end) as sent_today
      from leads
    `),
  ])
  ...
  const stats = statsRes.rows[0] || { total: 0, sent_total: 0, opened_total: 0, sent_today: 0 }
