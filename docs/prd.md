...
- Webhook requests are HMAC-verified against the signing secret (`RESEND_WEBHOOK_SECRET`, `whsec_…`) using the Standard Webhooks algorithm; unauthenticated requests are rejected with 401. Requests are also rejected with 401 when the env var is not set, so signature verification can never be silently disabled.
- `suppressed_emails` (email, reason `bounce`/`complaint`, created_at) is checked by the sender, follow-up, generation, website scraper, and no-website email sourcing so a dead address can never enter the pipeline again — even via re-discovery. The generation stage also skips suppressed addresses before calling the AI, so no tokens are wasted writing emails that would never be sent. Temporary (`Temporary`) bounces are deliberately not suppressed; those surface as `email.delivery_delayed` events.
...
- `leads` gains a nullable `bounced_at` column; `failed` is reused as the on-lead status so the dashboard needs no new filter tab. A new `suppressed_emails` table (email, reason `bounce`/`complaint`, created_at) stores dead addresses so they can never be re-sent or re-sourced.
...
- **Hard daily Places budget.** Every Google Places page fetch is one billable request, and the old loop re-ran discovery repeatedly (re-fetching already-known places) which burned the free credit fast. Discovery now reserves one call per page against a daily cap (`MAX_PLACES_CALLS_PER_DAY = 100`, configurable in `lib/constants.ts`) tracked on the settings row (`places_used_date` / `places_used_count`). When the budget is spent, discovery skips cleanly (`skipped: 'quota_exhausted'`) until the next day — it never fails or retries into the API. (The two columns are applied with idempotent `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` statements — see the README's Automation section.)
- **Quota errors short-circuit.** A `429` stops discovery immediately for the day instead of hammering the endpoint. A `403` also stops discovery, but is treated as a configuration error (e.g. API/billing not enabled) and surfaces in the run's `error` so the operator gets an alert rather than silent skips.
